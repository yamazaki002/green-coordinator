...
def control(sensors: SensorData) -> SwitchState:
    ...
    # --- Определяем уровень доступной генерации ---------------------------
    generation = _has_sufficient_generation(solar_avail, wind_strength, battery_level)

    # --- Цель 3: защита накопителя ----------------------------------------
    # Если батарея ниже критического уровня — отключаем её от нагрузки,
    # но оставляем доступной для подзарядки от возобновляемых источников.
    battery_usable = battery_level > BATTERY_DEEP_DISCHARGE

    # --- Включаем источники генерации -------------------------------------
    solar_on = solar_avail and _is_daytime(time)
    wind_on = wind_strength >= WIND_THRESHOLD_LOW
    # Батарея подключена если есть заряд и есть нагрузка (не разряд без нагрузки)
    battery_on = battery_usable  # уточняется ниже

    # --- Цель 1: больница всегда включена ---------------------------------
    hospital_on = True

    # --- Цель 2: школа в рабочее время ------------------------------------
    school_on = _is_school_hours(time)

    # --- Остальные потребители зависят от уровня генерации ----------------
    if generation == "high":
        houses_on = True
        factory_on = True
        ...
        # Режим дефицита: только приоритетные потребители
        houses_on = False
        factory_on = False

    # --- Защита от разряда без нагрузки -----------------------------------
    # Батарея не должна разряжаться, если ни один потребитель не подключён.
    any_consumer = hospital_on or school_on or houses_on or factory_on
    if not any_consumer:
        battery_on = False

    # ГЭС не используется (политика энергоэффективности)
    ges_on = False
...
def _validate_sensor(name: str, value, expected_type, ranges: dict) -> Tuple[bool, object]:
    """
...
def _sanitize_sensors(sensors: SensorData) -> Tuple[Optional[SensorData], List[str]]:
    """
    Валидирует и очищает входные данные датчиков.

    Возвращает:
      (sanitized, warnings) — очищенные данные и список предупреждений.
      Если данные критически некорректны — возвращает (None, warnings).
    """
    warnings = []
    sanitized = {}

    # Проверяем числовые поля
    numeric_fields = ["time", "wind_strength", "battery_level",
                      "hospital_demand", "school_demand", "houses_demand", "factory_demand"]
    for field in numeric_fields:
        if field not in sensors:
            warnings.append(f"Отсутствует датчик: {field}")
            # Безопасные значения по умолчанию
            defaults = {
                "time": 12, "wind_strength": 0.0, "battery_level": 50.0,
                "hospital_demand": 100.0, "school_demand": 50.0,
                "houses_demand": 50.0, "factory_demand": 50.0,
            }
            sanitized[field] = defaults[field]
        else:
            val = sensors[field]
            expected = int if field == "time" else float
            # Разрешаем int для float-полей (преобразуем)
            if field != "time" and isinstance(val, int):
                val = float(val)
            ok, clean = _validate_sensor(field, val, (int, float) if field != "time" else int,
                                         SENSOR_RANGES)
            if not ok:
                warnings.append(f"Некорректное значение датчика {field}: {val!r}")
                # Для уровня заряда используем безопасный минимум — критично!
    ...
    # Проверяем булев флаг солнца
    if "solar_avail" not in sensors:
        warnings.append("Отсутствует датчик: solar_avail")
        sanitized["solar_avail"] = False
    else:
        val = sensors["solar_avail"]
        if not isinstance(val, bool):
            warnings.append(f"Некорректный тип solar_avail: {val!r}, ожидался bool")
            # Преобразуем из 0/1 если возможно
            sanitized["solar_avail"] = bool(val)
        else:
            sanitized["solar_avail"] = val

    return SensorData(**sanitized), warnings  # type: ignore
...
def _cross_validate(sensors: SensorData) -> List[str]:
    ...
    Примеры аномалий:
    - Солнечная генерация ночью (time < 6 or time > 20)
    - Ветер 0 %, но ветрогенераторы показывают нагрузку
    - Заряд батареи растёт при отсутствии источников
    """
    anomalies = []
    ...
    # Слишком высокое потребление при нулевых источниках
    total_demand = (sensors["hospital_demand"] + sensors["school_demand"]
                    + sensors["houses_demand"] + sensors["factory_demand"])
    if total_demand > 400:
        anomalies.append(
...
def safe_control(sensors: SensorData) -> SwitchState:
    """
    ...
    3. При обнаружении критических аномалий — переход в режим безопасного
       минимума (только больница и защита накопителя).
    4. Недоверенные показания заменяются консервативными значениями.
    ...
    if warnings:
        # В реальной системе: логировать предупреждения в SIEM
        for w in warnings:
            print(f"[WARN] {w}")
    ...
    # --- Шаг 3: коррекция аномальных показаний ---------------------------
    # Если солнце сигнализирует ночью — игнорируем (ставим False)
    time = clean_sensors["time"]
    solar_safe = clean_sensors["solar_avail"] and _is_daytime(time)

    # --- Шаг 4: применяем штатный алгоритм с очищенными данными ----------
    ...
    # --- Шаг 5: жёсткая гарантия целей безопасности ----------------------
    # Цель 1: больница ВСЕГДА включена, независимо от алгоритма
    result["hospital"] = True

    # Цель 3: ГЭС остаётся выключена (энергоэффективность)
    result["ges"] = False
...
TEST_CASES = [
    ...
    ("Сит1: день, солнце+средний ветер, батарея 85%",
     SensorData(time=12, solar_avail=True,  wind_strength=80, battery_level=85,
                hospital_demand=100, school_demand=60, houses_demand=70, factory_demand=80)),

    ("Сит2: день, солнце+слабый ветер, батарея 80%",
     SensorData(time=14, solar_avail=True,  wind_strength=20, battery_level=80,
                hospital_demand=100, school_demand=60, houses_demand=70, factory_demand=80)),

    ("Сит3: день, нет солнца+сильный ветер, батарея 25%",
     SensorData(time=10, solar_avail=False, wind_strength=75, battery_level=25,
                hospital_demand=100, school_demand=60, houses_demand=70, factory_demand=80)),

    ("Сит4: день, нет солнца+слабый ветер, батарея 20%",
     SensorData(time=16, solar_avail=False, wind_strength=15, battery_level=20,
                hospital_demand=100, school_demand=60, houses_demand=70, factory_demand=80)),

    ("Сит5: ночь, нет солнца+средний ветер, батарея 90%",
     SensorData(time=2,  solar_avail=False, wind_strength=85, battery_level=90,
                hospital_demand=100, school_demand=0,  houses_demand=50, factory_demand=60)),

    ("Сит6: ночь, нет солнца+слабый ветер, батарея 75%",
     SensorData(time=23, solar_avail=False, wind_strength=10, battery_level=75,
                hospital_demand=100, school_demand=0,  houses_demand=50, factory_demand=60)),

    ("Сит7: ночь, нет солнца+сильный ветер, батарея 15%",
     SensorData(time=3,  solar_avail=False, wind_strength=70, battery_level=15,
                hospital_demand=100, school_demand=0,  houses_demand=50, factory_demand=60)),

    ("Сит8: ночь, нет солнца+слабый ветер, батарея 10%",
     SensorData(time=1,  solar_avail=False, wind_strength=5,  battery_level=10,
                hospital_demand=100, school_demand=0,  houses_demand=50, factory_demand=60)),
]

ATTACK_CASES = [
    ("Атака1: ложный сигнал — солнце ночью",
     SensorData(time=3, solar_avail=True, wind_strength=50, battery_level=60,
                hospital_demand=100, school_demand=40, houses_demand=60, factory_demand=50)),

    ("Атака2: некорректный уровень заряда батареи (−5%)",
     SensorData(time=12, solar_avail=True, wind_strength=60, battery_level=-5,
                hospital_demand=100, school_demand=60, houses_demand=70, factory_demand=50)),

    ("Атака3: завышенное потребление больницы (999%)",
     SensorData(time=12, solar_avail=True, wind_strength=60, battery_level=70,
                hospital_demand=999, school_demand=60, houses_demand=70, factory_demand=50)),
    ...
    ("Атака5: слишком высокое суммарное потребление",
     SensorData(time=12, solar_avail=True, wind_strength=60, battery_level=80,
                hospital_demand=100, school_demand=150, houses_demand=150, factory_demand=150)),
]
...
def _print_switch(state: SwitchState) -> str:
    ...
    parts = []
    for key, label in [("ges", "ГЭС"), ("solar", "Солнце"), ("wind", "Ветер"),
                       ("battery", "Батарея"), ("hospital", "Больница"),
                       ("school", "Школа"), ("houses", "Дома"), ("factory", "Завод")]:
